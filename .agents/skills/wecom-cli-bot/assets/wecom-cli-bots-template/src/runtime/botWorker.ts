...
6. **文档与记忆**（可跳过） — "是否需要文档管理和长期记忆？（Bot 产出的文档会自动保存并可检索）1.是 2.否"
7. **版本追踪**（仅上一步选1时问） — "文档需要版本追踪吗？1.是 2.否"
8. **特殊要求**（可跳过） — "还有其他规则或约束吗？比如PRD必须包含某些字段。（可回复'跳过'）"
9. **确认** — 汇总所有配置为简表，让用户确认或修改。
