...
1. **行业背景**（可跳过） — "先了解一下背景：你所在的公司/团队是什么？主营业务是什么？（可回复'跳过'）"
...
4. **输出风格** — "回复风格？1.正式专业 2.简洁直接 3.轻松随意。语言？默认中文还是英文？"
5. **交互模式** — "当需要澄清需求时，你希望机器人如何与你交互？1.逐句引导（一问一答，适合复杂需求） 2.批量引导（一次列出所有问题，你一次性回答，适合效率优先）"
6. **文档管理** — "是否需要管理文档？1.是 2.否"
7. **文档类型**（仅上一步选1时问） — "文档类型？（多选如1,2）1.PRD 2.设计文档 3.测试用例 4.会议纪要 5.其他"
8. **版本追踪**（可跳过） — "文档需要版本追踪吗？1.是 2.否"
9. **记忆需求**（可跳过） — "需要长期记忆吗？1.是 2.否"
10. **记忆类型**（仅上一步选1时问） — "记住哪类信息？（多选如1,2,3）1.产品决策记录 2.用户反馈 3.历史需求变更 4.技术约束/限制 5.竞品信息 6.其他（请补充）"
11. **特殊要求**（可跳过） — "还有其他规则或约束吗？比如PRD必须包含某些字段。（可回复'跳过'）"
12. **确认** — 汇总所有配置为简表，让用户确认或修改。
